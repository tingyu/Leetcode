 ...
 */
/*
这题是Duplicates I的变形体，不过比那个难写多了。
首先想到需要一个fakeHead. 然后关键是用两个指针，第一个指针p遍历整个LinkedList
在每一次新建一个p2 = p.next来找后面重复的，如果p2和p2.next重复，就一直移动
p2指针。自己想的时候关键点没想到的是需要加个判断，如果p2移动了，就说明
有重复的，此时p.next = p2.next。如果p2没有移动，说明没有重复的，此时需要
移动p
